...
def train_model(
    ...
    *,
    ...
    predictor = cross_val_predict_autogluon_image_dataset(
        dataset=data,
...
def sum_xval_folds(model, model_results_folder, num_cv_folds=5, verbose=1, **kwargs):
    ...
    for split_num in range(num_cv_folds):
        ...
        pred_probs_split = _load_pickle(
            get_pickle_file_name("test_pred_probs"), verbose=verbose
        )
        labels_split = _load_pickle(
            get_pickle_file_name("test_labels"), verbose=verbose
        )
        test_pred_features_split = _load_pickle(
            get_pickle_file_name("test_pred_features"), verbose=verbose
        )
    ...
    for split_num in range(num_cv_folds):
        ...
        images_split = _load_pickle(
            get_pickle_file_name("test_image_files"), verbose=verbose
        )
        indices_split = _load_pickle(
            get_pickle_file_name("test_indices"), verbose=verbose
        )
