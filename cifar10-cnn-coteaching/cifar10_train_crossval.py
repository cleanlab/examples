...
parser.add_argument(
    "--cv-seed", default=0, type=int, help="seed for determining the cv folds. "
)
...
parser.add_argument(
    "--cv", "--cv-fold", type=int, default=None, metavar="N", help="The fold to holdout"
)
parser.add_argument(
    "--cvn",
    "--cv-n-folds",
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.train_labels is not None:
        ...
        # convert key to relative path of images
        # e.g. /datasets/datasets/cifar10/cifar10/train/truck/9996_truck.png --> train/truck/9996_truck.png
        # this conversion is needed because the image paths in the noisy labels .json file can be different than the image paths for our training data
        get_relative_path = lambda f: "/".join(Path(f).parts[-3:])
        train_labels_dict = {
            get_relative_path(k): v for k, v in train_labels_dict.items()
        }

        train_dataset.imgs = [
            (
                fn,
                train_labels_dict[get_relative_path(fn)],
            )  # convert key to relative path of images
    ...
    if use_crossval:
        ...
        kf = StratifiedKFold(
            n_splits=cv_n_folds, shuffle=True, random_state=args.cv_seed
        )
        cv_train_idx, cv_holdout_idx = list(kf.split(range(len(labels)), labels))[
            cv_fold
        ]
        # Separate datasets
        ...
        if use_mask:
            ...
            # Mask labels
            train_dataset.imgs = [
                img for i, img in enumerate(train_dataset.imgs) if train_bool_mask[i]
            ]
            train_dataset.samples = train_dataset.imgs
            clean_class_counts = np.bincount([lab for img, lab in train_dataset.imgs])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Train for one epoch
        if args.coteaching:
            coteaching.adjust_learning_rate(optimizer, epoch, alpha_plan, beta1_plan)
            coteaching.adjust_learning_rate(optimizer2, epoch, alpha_plan, beta1_plan)
            coteaching.train(
                train_loader,
                epoch,
                model,
            ...
            print(
                "Epoch [%d/%d] Test Accuracy on the %s test images: Model1 "
                "%.4f %% Model2 %.4f %%"
                % (epoch + 1, args.epochs, len(val_dataset), val_acc1, val_acc2)
            )
        else:
        ...
        if (
            not args.turn_off_save_checkpoint and not args.multiprocessing_distributed
    ...
    if use_crossval:
        ...
        if not args.turn_off_save_checkpoint:  # Load best of saved checkpoints
            print(
                "=> loading best model_{}__fold_{}_best.pth.tar".format(
                    args.arch, cv_fold
                )
            )
...
def get_probs(loader, model, args):
    ...
    probs = np.concatenate(
        [
            ...
            if args.gpu is None
            else torch.nn.functional.softmax(z, dim=1).cpu().numpy()
            for z in outputs
        ]
    )
...
def save_checkpoint(
    state, is_best, filename="checkpoint.pth.tar", cv_fold=None, use_mask=False
):
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if epoch < int(150.0 / 350 * args.epochs):
        lr = args.lr
    elif epoch < int(250.0 / 350 * args.epochs):
        lr = args.lr * 0.1
    else:
        lr = args.lr * 0.01
...
def combine_folds(args):
    ...
    labels = [
        label
        for img, label in datasets.ImageFolder(os.path.join(args.data, "train/")).imgs
    ]
